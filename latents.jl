...
##
d = JSON.parsefile("/home/doshna/Downloads/latents_new.json")
...
low = low_share[:,1:3]
share_low = low_share[:,4:end]
high_share = reduce(hcat,moth["share_high_latent"])' |> x -> Float64.(x)
share_high = high_share[:,1:3]
high = high_share[:,4:end]
##
""" 
...
l = lines!(ax1,fr[2:200],abs.(fsl)[2:200,3] ./ N,color=:steelblue,linewidth=3)
h = lines!(ax1,fr[2:200],abs.(fsh)[2:200,3] ./ N,color=:firebrick,linewidth=3)
...
l = lines!(ax1,freqqs,abs.(h_low[:,3]),color=:steelblue,linewidth=3)
h = lines!(ax1,freqqs,abs.(h_high[:,3]),color=:firebrick,linewidth=3)
...
l = lines!(ax1,fr[2:200],abs.(lowft)[2:200,3] ./ N,color=:steelblue,linewidth=3)
g = vlines!(ax1,freqqs,color=:grey,alpha=0.5)
...
l = lines!(ax1,fr[2:200],abs.(lowft)[2:200,3] ./ N,color=:firebrick,linewidth=3)
g = vlines!(ax1,freqqs,color=:grey,alpha=0.5)
...
cohs_low = zeros(18,3,7)
cohs_high = zeros(18,3,7)
cohs_shared_low = zeros(18,3,7)
cohs_shared_high = zeros(18,3,7)
...
##
f = Figure(size=(1000,1000)) 
ax = Axis(f[1,1],xticks = ([1,2,3],["L1","L2","L3"]),yticks = (range(1,18,length=18),string.(freqqs)),
    title= "Mean Coherence Low Specific",ylabel="Tracking Frequency (Hz)")
h = heatmap!(ax,mean(cohs_low,dims=3)[:,:,1]',colorrange=(0.4,1),colormap=:viridis)
ax = Axis(f[1,2],xticks = ([1,2,3],["H1","H2","H3"]),yticks = (range(1,18,length=18),string.(freqqs)),
    title= "Mean Coherence High Specific",ylabel="Tracking Frequency (Hz)")
h = heatmap!(ax,mean(cohs_high,dims=3)[:,:,1]',colorrange=(0.4,1),colormap=:viridis)
ax = Axis(f[2,2],xticks = ([1,2,3],["S1","S2","S3"]),yticks = (range(1,18,length=18),string.(freqqs)),
    title= "Mean Coherence High Shared",ylabel="Tracking Frequency (Hz)")
h = heatmap!(ax,mean(cohs_shared_high,dims=3)[:,:,1]',colorrange=(0.4,1),colormap=:viridis)
ax = Axis(f[2,1],xticks = ([1,2,3],["S1","S2","S3"]),yticks = (range(1,18,length=18),string.(freqqs)),
    title= "Mean Coherence Low Shared",ylabel="Tracking Frequency (Hz)")
